...
interface MatchingBatchSlideProps {
  batch: Flashcard[];
  isDisabled: boolean;
  onSubmit: (value: MatchingAssignments) => void;
  question: MatchingQuestion;
  value: MatchingAssignments | undefined;
}

export function MatchingBatchSlide({
  batch,
  isDisabled,
  onSubmit,
  question,
  value,
}: MatchingBatchSlideProps) {
  ...
  const canContinue = question.slots.every(
    (slot) => assignments[slot.id] !== undefined
  );

  return (
    <QuestionShell feedback={null} title={question.headerText}>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-muted-foreground text-sm">
                Fill every slot, then press Continue to commit the batch.
              </p>
              <p className="text-muted-foreground text-sm">
                Batch size: {batch.length} cards
              </p>
            </div>
            <p className="text-muted-foreground text-sm">
